...
def test_returns_none_for_an_empty_iterable() -> None:
    assert select_next_task([], NOW, USER_TIMEZONE) is None
...
def test_excludes_future_start_tasks_before_ranking() -> None:
    ...
    assert (
        select_next_task([future_task, available_task], NOW, USER_TIMEZONE)
        is available_task
    )
    assert select_next_task([future_task], NOW, USER_TIMEZONE) is None


def test_includes_task_starting_at_selection_reference() -> None:
    starting_now_task = task("starting-now", start=NOW)

    assert select_next_task([starting_now_task], NOW, USER_TIMEZONE) is starting_now_task
...
def test_normalizes_date_only_and_offset_datetimes_to_utc() -> None:
    ...
    assert (
        select_next_task([offset_task, date_only_task], now_with_offset, USER_TIMEZONE)
        is date_only_task
    )


def test_interprets_naive_start_and_deadline_in_the_user_timezone() -> None:
    local_time = datetime.fromisoformat("2026-08-29T14:00:00")
    local_task = task(
        "local",
        start=local_time,
        deadline=local_time,
    )
    available_task = task("available", priority=TaskPriority.HIGH)

    assert (
        select_next_task([available_task, local_task], NOW, USER_TIMEZONE)
        is local_task
    )


def test_preserves_the_instant_of_offset_aware_datetimes() -> None:
    user_timezone = ZoneInfo("America/New_York")
    offset_task = task(
        "offset",
        start=datetime(2026, 8, 29, 15, 0, tzinfo=timezone(timedelta(hours=3))),
        priority=TaskPriority.HIGH,
    )
    available_task = task("available", priority=TaskPriority.LOW)

    assert (
        select_next_task([available_task, offset_task], NOW, user_timezone)
        is offset_task
    )
...
def test_overdue_task_outranks_non_overdue_task() -> None:
    ...
    assert (
        select_next_task([non_overdue_task, overdue_task], NOW, USER_TIMEZONE)
        is overdue_task
    )
...
def test_priority_orders_tasks_within_the_same_overdue_group() -> None:
    ...
    assert (
        select_next_task([low_task, normal_task, high_task], NOW, USER_TIMEZONE)
        is high_task
    )
    assert (
        select_next_task([low_task, normal_task], NOW, USER_TIMEZONE)
        is normal_task
    )
...
def test_nearest_present_deadline_outranks_missing_deadline() -> None:
    ...
    assert (
        select_next_task(
            [distant_overdue_task, nearby_overdue_task], NOW, USER_TIMEZONE
        )
        is nearby_overdue_task
    )
    assert (
        select_next_task([no_deadline_task, upcoming_task], NOW, USER_TIMEZONE)
        is upcoming_task
    )
...
def test_nearest_future_deadline_outranks_more_distant_future_deadline() -> None:
    ...
    assert (
        select_next_task([distant_task, nearby_task], NOW, USER_TIMEZONE)
        is nearby_task
    )
...
def test_earlier_present_start_outranks_missing_start() -> None:
    ...
    assert (
        select_next_task([later_start_task, earlier_start_task], NOW, USER_TIMEZONE)
        is earlier_start_task
    )
    assert (
        select_next_task([no_start_task, earlier_start_task], NOW, USER_TIMEZONE)
        is earlier_start_task
    )
...
def test_lexicographically_smaller_id_breaks_ties_independent_of_input_order() -> None:
    ...
    assert (
        select_next_task([second_task, first_task], NOW, USER_TIMEZONE)
        is first_task
    )
    assert (
        select_next_task([first_task, second_task], NOW, USER_TIMEZONE)
        is first_task
    )
...
def test_does_not_modify_the_input_list() -> None:
    ...
    select_next_task(tasks, NOW, USER_TIMEZONE)
